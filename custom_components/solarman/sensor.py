...
from .services import Inverter, register_services, vol
...
async def _do_setup_platform(
    hass: "HomeAssistant", config, async_add_entities: AddEntitiesCallback
):
    ...
    hass_sensors = []
    async for sensor in inverter.get_sensors():
        try:
            if "isstr" in sensor:
                hass_sensors.append(
                    SolarmanSensorText(
                        inverter_name, inverter, sensor, inverter_sn
                    )
                )
            else:
                hass_sensors.append(
    ...
    hass_sensors.append(
        SolarmanStatusDiag(
            inverter_name, inverter, "status_lastUpdate", inverter_sn
        )
    )
    hass_sensors.append(
        SolarmanStatusDiag(
            inverter_name, inverter, "status_connection", inverter_sn
        )
    )
...
#  It provides device_info to Home Assistant which allows grouping all
...
class SolarmanStatus(SolarmanSensor, Entity):
    ...
    def unique_id(self):
        ...
        return "{}_{}_{}".format(
            self._inverter_name, self._sn, self._field_name
        )
...
#   Overrides the Status entity, supply the configured icon, and updates
...
class SolarmanSensorText(SolarmanStatus):
    def __init__(self, inverter_name, inverter, sensor, sn):
        SolarmanStatus.__init__(
            self, inverter_name, inverter, sensor["name"], sn
        )
        if "icon" in sensor:
...
#   Overrides the Text sensor and supply the device class, last_reset and unit
#   of measurement
##############################################################################
