class ParameterParser:
        ...
        if "min" in rule:
            if rule["min"] > value:
                if "invalidate_all" in rule:
                    raise ValueError(
                        f"Invalidate complete dataset ({title} ~ {value})"
                    )
                return False
    ...
    def try_parse_signed(self, rawData, definition, start, length):
        ...
        if found:
            ...
            if ("scale_division" in definition) and (
                definition["scale_division"] > 0
            ):
                value //= definition["scale_division"]

            if "validation" in definition:
                if not self.do_validate(
                    title, value, definition["validation"]
                ):
                    return
    ...
    def try_parse_unsigned(self, rawData, definition, start, length):
        ...
        if found:
            ...
            if "lookup" in definition:
                self.result[title] = self.lookup_value(
