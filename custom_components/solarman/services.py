...
SERVICE_READ_REGISTER_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(PARAM_REGISTER): vol.All(
...
SERVICE_READ_MULTIPLE_REGISTERS_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(PARAM_COUNT): vol.All(
            vol.Coerce(int), vol.Range(min=0, max=65535)
        ),
    }
)
...
SERVICE_WRITE_REGISTER_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(PARAM_VALUE): vol.All(
            vol.Coerce(int), vol.Range(min=0, max=65535)
        ),
    }
)
...
def register_services(hass: HomeAssistant):
    ...
    async def read_holding_register(call) -> int:
        if (inverter := get_inverter(call.data.get(PARAM_DEVICE))) is None:
        ...
        try:
            response = inverter.service_read_holding_register(
                register=call.data.get(PARAM_REGISTER)
            )
        except Exception as e:
            raise ServiceValidationError(
                e, translation_domain=DOMAIN, translation_key="call_failed"
            )
    ...
    hass.services.async_register(
        DOMAIN,
        SERVICE_READ_MULTIPLE_HOLDING_REGISTERS,
        read_multiple_holding_registers,
        schema=SERVICE_READ_MULTIPLE_REGISTERS_SCHEMA,
        supports_response=SupportsResponse.OPTIONAL,
    )

    hass.services.async_register(
        DOMAIN,
        SERVICE_WRITE_HOLDING_REGISTER,
